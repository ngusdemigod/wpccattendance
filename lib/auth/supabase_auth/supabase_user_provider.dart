...
class AttendamceSupabaseUser extends BaseAuthUser {
  AttendamceSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> attendamceSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AttendamceSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
